...
const init = () => {
	...
	//use baked lights or cheap light
	// avoid adding light or removeLight = it recompiles
	// avoid shadow , use baked shadow
	...
	// // Tip 11 use cast, recieve shadow wisely = less render in shadow map
	// cube.castShadow = true
	...
	// renderer.shadowMap.needsUpdate = true //by this only once update

	// 13 : texture takes lots of memory in gpu , mipmaps... only resoultion matters
	// 14 : keep power of 2 resolution
	// // Tip 18  : mutualize geo
	// for(let i = 0; i < 50; i++)
	// {
	...
	// //if u r going to change matrices in tick function do upper
	// scene.add(mesh);
	// for (let i = 0; i < 50; i++) {
	...
	//23 low poly models , if need detail , use normal maps

	//24 gZip such as , gltf , etc..
	//25 camera fov <- control , if not in camera , wont be renderered.

	// // Tip 29
	// renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
	// 30 powerPreference : on renderer

	// 31 antialias => if not needed, disable
	// // Tip 31, 32, 34 and 35

	//try to reduce as much pass as passibel
	//1920 * 1080 resolution with 4 pass + pixelRatio 2
	// == 1920 *1080 * 4 * 2 * 2 (w , h)

	//no if in vertex shader

	// use define rather uniform , 미미함

	// change precision to low

	// if possible , do the calculations in vertex shader
